...
func (e *Element) writeTo(w *bufio.Writer, s *WriteSettings, depth int, indent indentFunc, align bool) {
	...
	if len(e.Child) > 0 {
		...
		for _, c := range e.Child {
			...
			c.writeTo(w, s, depth+1, indent, !prevNodeHasTail && !prevNodeWasText && align && e.Tag != "p")
			prevNodeHasTail = len(c.tail()) > 0 && !isWhitespace(c.tail())
			prevNodeWasText = isCharData
		}
		if haveNonCharacterNodes && !prevNodeHasTail && !prevNodeWasText && depth >= 0 && indent != nil && align && e.Tag != "p" {
			w.WriteString(indent(depth))
		}
		w.Write([]byte{'<', '/'})
		if e.Space != "" {
